...
- (void)requestTimesForFilter:(FerryFilter*)filter callback:(void (^)(NSDictionary* results))callbackBlock{
    ...
    [self.requestManager GET:[NSString stringWithFormat:@"wp-json/wp/v2/posts?include=%@/%@", filter.selectedLocation.serverId, [formatter stringFromDate:filter.selectedDate]] parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
